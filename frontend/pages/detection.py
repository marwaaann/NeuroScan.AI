...
def render_detection_page(is_api_connected: bool):
    ...
    render_top_header(
        title="NeuroScan.AI MRI Detection Studio",
    ...
    with col_left:
        ...
        active_bytes = st.session_state.get("active_bytes")
        active_filename = st.session_state.get("active_filename", "mri_scan.jpg")
    ...
    with col_right:
        ...
        if run_analysis and active_bytes:
            with st.spinner("Analyzing cranial MRI scan with YOLOv8..."):
                ...
                if success:
                    ...
                    is_tumor = response_data.get("is_tumor_detected", False)
                    tumor_count = response_data.get("count", 0)
                    tumor_dets = response_data.get("tumor_detections", [])
                    primary_class = tumor_dets[0]["class_name"] if is_tumor and tumor_dets else "No Tumor"
                    primary_conf = tumor_dets[0]["confidence"] if is_tumor and tumor_dets else (response_data.get("healthy_confidence") or 0.0)
        ...
        # State 3: Scan loaded, awaiting analysis trigger
        else:
